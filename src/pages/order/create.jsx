...
const OrderInvoice = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch customers
        const customerResponse = await axios.get('http://localhost/Laravel/Laravel_POS/public/api/find_customer');
        console.log(customerResponse);
        
        setCustomers(customerResponse.data.customer); // assuming the response contains customer data under 'customer'

        // Fetch products
        const productResponse = await axios.get('http://localhost/Laravel/Laravel_POS/public/api/find_product');
        console.log(productResponse);
        
        setProducts(productResponse.data.product); // assuming the response contains product data under 'product'
      } catch (error) {
  ...
  const getTax = () => {
    return getTotal() * 0.05; // Assuming a 5% tax
  };
  ...
  return (
    <div className="container py-4">
        ...
        <table className="table table-bordered table-hover">
            ...
            <tr>
              ...
              <td>
                <input
                  type="text"
                  className="form-control"
                  value={(selectedProduct ? selectedProduct.price * productQty - productDiscount : 0).toFixed(2)}
                  readOnly
                />
              </td>
          ...
          <tfoot>
            ...
            <tr>
              ...
              <td>${(cart.reduce((acc, item) => acc + parseFloat(item.discount), 0)).toFixed(2)}</td>
            </tr>
            <tr>
              <td colSpan="5" className="text-end fw-bold">Total</td>
              <td className="fw-bold">${getGrandTotal().toFixed(2)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
      ...
      <div className="d-flex justify-content-end">
        <button className="btn btn-success btn-lg">Process</button>
      </div>
    </div>
  );
};
